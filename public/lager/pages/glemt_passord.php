...
//
// I tillegg:
// - Hvis bruker ALLEREDE er innlogget og must_change_password=1 (f.eks. innlogget med midlertidig passord),
//   så hopper vi direkte til steg 3 og bruker innlogget bruker-id.
//
...
// 0) Hvis bruker er innlogget og must_change_password=1 -> tving steg 3
$loggedIn = lager_user(); // forventet fra bootstrap.php
if (is_array($loggedIn) && !empty($loggedIn['id']) && col_exists($pdo, 'lager_users', 'must_change_password')) {
    $uid = (int)$loggedIn['id'];
    $dbu = find_user_by_id($pdo, $uid);
    if ($dbu && (int)($dbu['must_change_password'] ?? 0) === 1) {
        $forceChange = true;
        $_SESSION['pwreset_verified_uid'] = $uid;
        // lagre noe ident-ish hvis vi har
        if (!empty($dbu['email'])) {
            $_SESSION['pwreset_verified_ident'] = (string)$dbu['email'];
        } elseif (!empty($dbu['username'])) {
            $_SESSION['pwreset_verified_ident'] = (string)$dbu['username'];
        }
        $step = 'setnew';
        $ok = 'Du er logget inn med midlertidig passord. Velg et nytt passord for å fortsette.';
    }
}

// 1) Hvis vi allerede har verifisert bruker i denne sesjonen:
if (!$forceChange && !empty($_SESSION['pwreset_verified_uid'])) {
    $step = 'setnew';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'send_tmp') {
        ...
        if ($ident === '') {
            ...
            // Behold ident i neste steg (kun UI)
            $_POST['ident'] = $ident;
        }
    }
    ...
    // 3) Sett nytt passord (gjelder både "verifisert via SMS" og "tvang ved innlogging")
    if ($action === 'set_new') {
        $uid = (int)($_SESSION['pwreset_verified_uid'] ?? 0);

        // Hvis bruker er innlogget og vi fortsatt mangler uid, bruk innlogget id som fallback
        if ($uid <= 0) {
            $li = lager_user();
        ...
        if ($uid <= 0) {
            ...
            if (strlen($pw1) < 8) {
                ...
                try {
                    ...
                    // Rydd sesjonsdata for reset-flyt (men behold innlogging om bruker er logget inn)
                    unset($_SESSION['pwreset_verified_uid'], $_SESSION['pwreset_verified_ident'], $_SESSION['pwreset_csrf']);

                    // Hvis dette var tvungen endring etter innlogging: send videre til lager
                    $li = lager_user();
                    if (is_array($li) && !empty($li['id'])) {
                        redirect('/lager');
                        exit;
